...
const HistoryScreen = ({route}) => {
  ...
  const [isTheEndOfTheList, setTheEndOfTheList] = useState(false);
  ...
  useEffect(() => {
    if (historyList.length === historyTotalItems.current) {
      setTheEndOfTheList(true);
    }
  }, [historyList]);

  const _fetchHistory = useCallback(async (page) => {
    try {
      if (!isTheEndOfTheList) {
        const data = await getHistory(page * LIMIT, LIMIT);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <FlatList
        ...
        refreshControl={
          <RefreshControl refreshing={isRefreshing} onRefresh={_handleOnRefresh} />
        }
      />
    </SafeAreaView>
  );
